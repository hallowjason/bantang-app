...
const TYPE_COLOR: Record<EventType, string> = {
  ...
  other:   'bg-[var(--color-hairline)] text-muted',
}
...
export default function PortalEvents() {
  ...
  return (
    <div className="max-w-lg mx-auto px-4 pt-5 flex flex-col gap-5">
      ...
      <h2 className="text-sm font-semibold text-ink px-1">活動意願調查</h2>
      ...
      {loading ? (
        ...
        <div className="card-lovable text-center">
          <p className="text-sm text-muted">目前沒有進行中的活動</p>
          <p className="text-xs text-muted mt-1 opacity-70">請稍後再回來查看</p>
        </div>
      ) : (
        <>
          {/* 進行中 */}
          ...
          {pastEvents.length > 0 && (
            <div className="flex flex-col gap-3">
              <h3 className="text-xs font-semibold text-muted px-1">已截止</h3>
...
function EventCard({ event, past = false }: { event: PortalEvent; past?: boolean }) {
  return (
    <Link
      ...
      className={`bg-cream border border-hairline rounded-xl overflow-hidden flex flex-col hover:shadow-sm transition-shadow ${past ? 'opacity-60' : ''}`}
    >
      {/* 封面圖片 */}
      {event.imageUrl && (
        <img
      ...
      <div className="px-5 py-4 flex flex-col gap-2">
        ...
        {event.eventDates?.length > 0 && (
          <p className="text-xs text-muted">{formatDates(event.eventDates)}</p>
        )}
        ...
        <div className="flex items-center justify-between pt-1">
          <span className={`text-xs ${past ? 'text-muted' : 'text-red-500'}`}>
            截止：{event.deadline.replace(/-/g, '/')}
            {past ? '（已截止）' : ''}
          </span>
          {!past && (
            <span className="text-xs text-ink font-medium">填寫意願 →</span>
          )}
        </div>
      </div>
    </Link>
