 ...
 *   - head_leader (and class_master) can enter; sees "領班管理後台"
 *   - leader is redirected to /attendance
 *
 * Covers:
 *   1. head_leader 進入 /admin 看到「領班管理後台」
 *   2. leader 進入 /admin 被導向 /attendance
 */

async function goToAdmin(page: Page, role: 'leader' | 'head_leader') {
  const classId = uniqueClassId('e2e-dash')
  await seedClass(classId, 'E2E Dash Class')
  const user = role === 'head_leader' ? TEST_USERS.headLeader : TEST_USERS.leader
...
test.describe('Admin — /admin 角色 gating', () => {
  ...
  test('head_leader 進入 /admin 看到管理後台標題', async ({ page }) => {
    await goToAdmin(page, 'head_leader')
